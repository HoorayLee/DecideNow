...
class FortuneWheelViewModel: ObservableObject {
    ...
    private func getWheelStopDegree() -> Double {
        ...
        if index < 0 || index >= model.options.count { index = Int.random(in: 0..<model.options.count) }
        index = model.options.count - index - 1;
        /*
         ...
         */
        let itemRange = 360 / model.options.count;
    ...
    func spinWheel() {
        ...
        let requestWorkItem = DispatchWorkItem { [weak self] in
            ...
            let count = self.model.options.count
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + model.animDuration + 0.2, execute: requestWorkItem)
    }
}
